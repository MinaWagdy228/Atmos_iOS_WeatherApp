...
struct AppTheme {
    ...
    var secondaryText: Color {
        timeOfDay == .day
            ? Color.black.opacity(0.65) : Color.white.opacity(0.75)
    }

    var captionText: Color {
        timeOfDay == .day
            ? Color.black.opacity(0.45) : Color.white.opacity(0.50)
    }
    ...
    var cardStroke: Color {
        timeOfDay == .day
            ? Color.white.opacity(0.50) : Color.white.opacity(0.18)
    }

    var divider: Color {
        timeOfDay == .day
            ? Color.black.opacity(0.08) : Color.white.opacity(0.12)
    }
    // MARK: - Animation Tokens

    var shimmerHighlight: Color {
        // A clean, soft light reflection for dark mode, and a soft dark tint for bright light mode
        timeOfDay == .day
            ? Color.black.opacity(0.08) : Color.white.opacity(0.35)
    }
